...
function selectmode:update(dt)
	...
	if love.mouse.getY() < (love.graphics.getHeight()/2 - start.font2:getHeight( "SURVIVAL MODE" )/2 - 245) - 35 then
		self.survivalstatemouse = false
		self.arcadestatemouse = false
		self.endlessstatemouse = false
		self.stuckstatemouse = false
	...
	if love.mouse.getY() > (love.graphics.getHeight()/2 - start.font2:getHeight( "STUCKINTHEMUD" )/2 + 55 + start.font2:getHeight( "STUCKINTHEMUD" )) + 35 then
		self.survivalstatemouse = false
		self.arcadestatemouse = false
		self.endlessstatemouse = false
		self.stuckstatemouse = false
	...
	if love.mouse.getX() > (love.graphics.getWidth()/2 - 290) - start.font2:getWidth( "<" )/2 + start.font2:getWidth( "<" ) + 30 then
		self.backstatemouse = false
		self.mouseoverback = false
	end 
	
	if love.mouse.getX() < (love.graphics.getWidth()/2 - 310) - start.font2:getWidth( "<" )/2 - 40 then
		self.backstatemouse = false
		self.mouseoverback = false
	end
...
function selectmode:keypressed(key)
	...
	if key == "return" and self.survivalstate == true or key == "space" and self.survivalstate == true or key == 'escape' and self.survival == true then
	  ...
	  -- Go to the endelss game mode
	if key == "return" and self.endlessstate == true or key == "space" and self.endlessstate == true then
	...
	if key == 'escape' and self.survival == false  then
		Gamestate.pop()
		love.audio.play(self.backsound)
		self.arrowy = 100
	end
...
function selectmode:mousepressed(mx, my, button)
	...
	if button == 1 and self.endlessstatemouse == true and self.backstatemouse == false and self.mouseover == true then
		Gamestate.push(endless)
		game.endless = true
		love.audio.play(self.entersound1)
		love.audio.stop(start.music)
	...
	-- Go back to main menu
	if button == 2 and self.survival == false then
		Gamestate.pop()
		love.audio.play(self.backsound)
		self.arrowy = 100
	end
